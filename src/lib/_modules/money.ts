...
export function computeBalances(allExpensesObject, allMembers, allDonePaymentsObject) {
	...
	const total = allExpenses.map((x) => x[1].amount).reduce((a, b) => a + b, 0);
	const numMembers = allMembers.length;
	const eachUserBalance = {};
	const payments = {}; // payments["charles"] = [["cryptoboid", 10]]

	for (const member of allMembers) {
		// if (member[1] === NaN) return {};
		eachUserBalance[member[1].name] = 0;
		payments[member[0]] = [];
	}
	...
	for (const [usr, balance] of Object.entries(eachUserBalance)) {
...
export function computePayments(balance: [string, unknown][]): any {
	const sortedMostGenerous = balance.map((x) => Array.from(x));
	let currMostGen = 0,
	...
	while (currMostGen < currLeastGen) {
		const mostGen = sortedMostGenerous[currMostGen];
		const leastGen = sortedMostGenerous[currLeastGen];
...
export function recordPayment(payerName: string, receiverName: string, payedAmount: number) {
	const groupStoreRef = get(groupStore);
